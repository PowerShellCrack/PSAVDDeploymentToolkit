...
Function Compress-7zipArchive {
    ...
    if ($result.ExitCode -eq 0) {
        ...
        Write-Host ("Error occurred while archiving the folder. {0}" -f $result.ExitCode)
    }
}
...
Function Expand-7zipArchive{
    ...
    if ($result.ExitCode -eq 0) {
        Write-Host "Folder expanded successfully."
    } else {
        Write-Host ("Error occurred while expanding the folder. {0}" -f $result.ExitCode)
    }
}
